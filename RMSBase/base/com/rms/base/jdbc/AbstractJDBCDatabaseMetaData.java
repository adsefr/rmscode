...
abstract class AbstractJDBCDatabaseMetaData implements JDBCDataBaseMetaData {
	...
	private void setCatalogMetas() throws SQLException {
		...
		catalogQueryResult = JDBCUtil.getJDBCQueryResult(databaseMetaData.getCatalogs());
		while (catalogQueryResult.hasNext()) {
			DefaultCatalogMeta catalogMeta = (DefaultCatalogMeta) JDBCUtil.getCatalogMeta();
	...
	private void setSchemaMetas(CatalogMeta catalogMeta) throws SQLException {
		...
		schemaQueryResult = JDBCUtil.getJDBCQueryResult(databaseMetaData.getSchemas(catalogName, null));
		while (schemaQueryResult.hasNext()) {
			DefaultSchemaMeta schemaMeta = (DefaultSchemaMeta) JDBCUtil.getSchemaMeta();
	...
	private void setTableMetas(SchemaMeta schemaMeta) throws SQLException {
		...
		tableQueryResult = JDBCUtil.getJDBCQueryResult(databaseMetaData.getTables(catalogName, schemaName, null, null));
		...
		while (tableQueryResult.hasNext()) {
			...
			DefaultTableMeta tableMeta = (DefaultTableMeta) JDBCUtil.getTableMeta();
	...
	private void setColumnMetas(TableMeta tableMeta) throws SQLException {
		...
		columnQueryResult = JDBCUtil.getJDBCQueryResult(databaseMetaData.getColumns(catalogName, schemaName, tableName, null));
		...
		while (columnQueryResult.hasNext()) {
			...
			DefaultColumnMeta columnMeta = (DefaultColumnMeta) JDBCUtil.getColumnMeta();
	...
	private void setPrimaryKeys(TableMeta tableMeta) throws SQLException {
		...
		primaryKeysQueryResult = JDBCUtil.getJDBCQueryResult(databaseMetaData.getPrimaryKeys(catalogName, schemaName, tableName));
